    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                ...
                <form class="custom-validation" method="POST" action="{{ route('admin.letters.update', $letter->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    <div class="form-group row">
                        <label for="name" class="col-sm-2 col-form-label">Nama Surat *</label>
                        <div class="col-sm-10">
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name') ?? $letter->name }}" data-parsley-length="[5,255]" required>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="name" class="col-sm-2 col-form-label">Status *</label>
                        <div class="col-sm-10">
                            <input type="checkbox" name="status" id="switch3" switch="bool" {{ $letter->status == 'On' ? 'checked' : '' }}/>
