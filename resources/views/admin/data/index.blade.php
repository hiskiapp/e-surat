            ...
            <tr>
              <td>
                <a class="image-popup-no-margins" href="{{ asset($admin->photo) }}">
                  <img class="img-fluid" alt="{{ $admin->name }}" src="{{ asset($admin->photo) }}" width="24">
                </a>
              </td>
              <td>{{ $admin->name }}</td>
              <td>{{ $admin->username }}</td>
              <td>
                <a class="btn btn-sm btn-warning waves-effect waves-light" href="{{ route('admin.data.edit', $admin->id) }}" role="button"><i class="mdi mdi-grease-pencil"></i></a>
                <form method="POST" action="{{ route('admin.data.destroy', $admin->id) }}" class="d-inline form-delete">
                  @csrf
                  @method('DELETE')
                  <button type="submit" class="btn btn-sm btn-danger waves-effect waves-light"><i class="mdi mdi-trash-can"></i></button>
                </form>
                <a class="btn btn-sm btn-info waves-effect waves-light" href="{{ route('admin.data.show', $admin->id) }}" role="button"><i class="mdi mdi mdi-eye-circle"></i></a>
              </td>
            </tr>
            @endforeach
          </tbody>
